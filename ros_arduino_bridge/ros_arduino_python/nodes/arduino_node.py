...
"""
# Se adapta el software para que el envio sea a traves de una red Wifi
# Modificaciones realizadas por Alberto Diaz Rodriguez
...
class ArduinoROS():
    def __init__(self):
        ...
        self.IP = rospy.get_param("~IP", "192.168.43.168")
        ...
        self.controller = Arduino(self.IP, self.socket, self.timeout, self.motors_reversed)

        # Make the connection
        self.controller.connect()

        rospy.loginfo("Connected to Arduino on port " + self.IP + " at " + str(self.socket) + " baud")
	...
	self.controller.motors_speed(data.x,data.y,data.z)
        rospy.loginfo("El mensaje recibido es: [%f,%f,%f]", data.x,data.y,data.z)
        return
